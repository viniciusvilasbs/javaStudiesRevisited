...
public class StreamTest13 {
    ...
    public static void main(String[] args) {
        Map<Promotion, List<LightNovel>> groupByPromotion = lightNovels.stream()
                .collect(Collectors.groupingBy(
                        ln -> ln.getPrice() < 10 ? Promotion.UNDER_PROMOTION : Promotion.REGULAR_PRICE));
        System.out.println(groupByPromotion);

        System.out.println("**********************");

        Map<Category, Map<Promotion, List<LightNovel>>> groupByCategoryAndPromotion = lightNovels.stream()
                .collect(Collectors.groupingBy(LightNovel::getCategory,
                       Collectors.groupingBy(
                               ln -> ln.getPrice() < 10 ? Promotion.UNDER_PROMOTION : Promotion.REGULAR_PRICE)));

        System.out.println(groupByCategoryAndPromotion);
    }
}
